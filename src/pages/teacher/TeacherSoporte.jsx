...
import { fetchGroups, fetchSupportRequests, createSupportRequest } from '../../lib/db';
    ...
    <div className="admin-modal-overlay" onClick={onClose}>
      <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="admin-field">
          ...
          <textarea rows={4} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Ej. No puedo subir un material a un módulo..." />
        </div>
...
const TeacherSoporte = () => {
  ...
  const reserveTutoring = async (course, group) => {
    await createSupportRequest({
      requesterUid: currentUser?.uid, requesterName, requesterRole: 'teacher', type: 'tutoria',
  ...
  const courseGroups = courses.map((c) => ({ course: c, group: groups.find((g) => g.courseId?.toString() === c.id.toString()) }));
        ...
        <div>
          ...
          <div className="admin-banner" style={{ marginBottom: 0 }}>
            <div>
              ...
              <button className="admin-btn-edit" style={{ marginTop: 14 }} onClick={() => navigate('/teacher/cursos')}><Sparkles size={14} /> Entrar a un curso</button>
            </div>
          </div>
        </div>
      </div>
